    ...
    if(!_cancelPublish){
        if(result.retCode == 0){
            ...
            /*
               ...
               NSDictionary *dic = @{@"token":SafeStr(token),
                                     @"content":SafeStr(self.videoTitleView.text),
                                     @"videoPath":SafeStr(videoUrl),
                                     @"videoSeconds":@(self.videoInfo.duration),
                                     @"videoWidth":@(self.videoInfo.width),
                                     @"videoHeight":@(self.videoInfo.height),
                                     @"coverPath":SafeStr(result.coverURL)
                                     };
               AA_WEAK_MAKE(self) weakSelf = self;
               HttpRequest *quest = [HttpRequest buildRequest:@"/api/videos/releaseVideos"];
               [quest post:dic success:^(id data) {
                   weakSelf.generationView.hidden = YES;
                   weakSelf.generateProgressView.progress = 0;
