...
-(void)initUI
{
    ...
    _btnRatio916 = [SmallButton buttonWithType:UIButtonTypeCustom];
    _btnRatio916.frame = CGRectOffset(_btnRatioFrame, -(30 + BUTTON_CONTROL_SIZE), 0);
    [_btnRatio916 setImage:[UIImage imageNamed:@"916"] forState:UIControlStateNormal];
    [_btnRatio916 setImage:[UIImage imageNamed:@"916_hover"] forState:UIControlStateHighlighted];
    [_btnRatio916 addTarget:self action:@selector(onBtnRatioClicked:) forControlEvents:UIControlEventTouchUpInside];
    _btnRatio916.tag = VIDEO_ASPECT_RATIO_16_9;
    _btnRatio916.hidden = YES;
    [self.view addSubview:_btnRatio916];
    
    _btnRatio11 = [SmallButton buttonWithType:UIButtonTypeCustom];
    _btnRatio11.frame = CGRectOffset(_btnRatio916.frame, -(30 + BUTTON_CONTROL_SIZE), 0);
    ...
    switch (_videoConfig.videoRatio) {
        ...
        case VIDEO_ASPECT_RATIO_16_9:
            [self onBtnRatioClicked:_btnRatio916];
            break;
        default:
            break;
    }
    ...
    switch (btn.tag) {
        case VIDEO_ASPECT_RATIO_9_16:
        {
            if (btn.right + 20 == self.view.frame.size.width && [self ratioIsClosure]) {
                _btnRatio916.frame = CGRectOffset(btn.frame, -(30 + BUTTON_CONTROL_SIZE), 0);
                _btnRatio11.frame = CGRectOffset(_btnRatio916.frame, -(30 + BUTTON_CONTROL_SIZE), 0);
                _btnRatio43.frame = CGRectOffset(_btnRatio11.frame, -(30 + BUTTON_CONTROL_SIZE), 0);
                _btnRatio916.hidden = NO;
                _btnRatio11.hidden = NO;
                _btnRatio43.hidden = NO;
            }else{
                btn.frame = _btnRatioFrame;
                _btnRatio916.frame = _btnRatioFrame;
        ...
        case VIDEO_ASPECT_RATIO_16_9:
        {
            ...
            CGFloat height = _videoRecordView.frame.size.width * 9 / 16;
            [UIView animateWithDuration:0.2 animations:^{
                _videoRecordView.frame = CGRectMake(0, (self.view.frame.size.height - height) / 2.0, _videoRecordView.frame.size.width, height);;
            }];
            _aspectRatio = VIDEO_ASPECT_RATIO_16_9;
            [[TXUGCRecord shareInstance] setAspectRatio:_aspectRatio];
        }
            break;
        case VIDEO_ASPECT_RATIO_1_1:
        {
            if (btn.right + 20 == self.view.frame.size.width && [self ratioIsClosure]) {
                ...
                _btnRatio169.frame = CGRectOffset(_btnRatio43.frame, -(30 + BUTTON_CONTROL_SIZE), 0);
                _btnRatio916.frame = CGRectOffset(_btnRatio169.frame, -(30 + BUTTON_CONTROL_SIZE), 0);
                _btnRatio43.hidden = NO;
                _btnRatio169.hidden = NO;
                _btnRatio916.hidden = NO;
            }else{
