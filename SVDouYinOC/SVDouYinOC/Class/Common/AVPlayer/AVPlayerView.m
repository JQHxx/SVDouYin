...
-(void)setPlayerWithUrl:(NSString *)url {
    ...
    _queryCacheOperation = [[WebCacheHelpler sharedWebCache] queryURLFromDiskMemory:_cacheFileKey cacheQueryCompletedBlock:^(id data, BOOL hasCache) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [weakSelf.urlAsset.resourceLoader setDelegate:weakSelf queue:self.queue];
            //初始化AVPlayerItem
...
- (CGFloat)duration {
    ...
    return CMTimeGetSeconds(_playerItem.asset.duration);
}

- (CGFloat)fps {
    if([[_playerItem.asset tracksWithMediaType:AVMediaTypeVideo] count]) {
        return  [[[_playerItem.asset tracksWithMediaType:AVMediaTypeVideo] objectAtIndex:0] nominalFrameRate];
    }
    return 0.0;
}
